  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      try {
        ...
        setFavorites(response.data.map(fav => fav.recipeId)); // Make sure recipeId exists
      } catch (error) {
      ...
      >
        {favorites.length === 0 ? (
          ...
          favorites.map((item, index) => {
            if (!item) {
              return null; // Skip if item is null or undefined
            }
            return (
              <div
                className="col-12 col-sm-6 col-md-4 col-lg-3"
                key={item._id || index} // Use index as fallback if _id is missing
              >
